...
GLdouble l, r, b, t;
GLfloat speed_horizontal = 0.0009f; //speedX
GLfloat speed_vertical = 0.0009f; //speedY
int p, q;
int flag = 0, level = 1;
...
void display() {
    ...
    if (flag == 0)
    {
       ...
       /* case 4: score_level = "04"; break;
        ...
        if (f <= 0.424264068 && y <= -0.5 && y >= -0.7) {
            ...
            if (touch_ball_board == 4) { //after 5 touch 
                touch_ball_board = 0;
                level++; // next level
                speed_horizontal = 1.07 * speed_horizontal; //incr the speed of ball in x-axis
                speed_vertical = 1.07 * speed_vertical; //incr the speed of ball in y-axis
            }
        }
        if ((y < -0.55 && f < 0.5831 && f > 0.5)) {
            x = x; y = y; speed_horizontal = 0; speed_vertical = 0; flag = 1; // game over
    ...
    else
    {
        ...
        glPushMatrix();
        glColor3f(0.0, 0.0, 0.0);
        glTranslatef(-0.15, 0.15, 0.00);
        print(0, 0, 0, "Game Over");
        glPopMatrix();
        glPushMatrix();
        glColor3f(0.0, 0.0, 0.0);
        glTranslatef(-0.30, -0.15, 0.00);
        print(0, 0, 0, " Done by : fantastic group ");
        glPopMatrix();
        glPushMatrix();
        glColor3f(0.0, 0.0, 0.0);
        glTranslatef(-0.80, -0.30, 0.00);
        print(0, 0, 0, " Arwa & lubaba & Sarah & bothainah & Ghaida & Fatma ");
        glPopMatrix();
        glutSwapBuffers();
    }
}
